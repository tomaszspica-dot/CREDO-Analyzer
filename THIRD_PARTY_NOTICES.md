# Third-party notices

This repository contains project-local code and explicitly identified
third-party material.

## CREDO API Tools

The file:

`app/credo-data-exporter_universal.py`

originates from:

`credo-science/credo-api-tools`

Upstream path:

`data-exporter/credo-data-exporter_universal.py`

Verified upstream Git blob:

`b4b09dcdba11e1db9d2215b6e8b4cb60589be671`

License:

MIT

The corresponding license text is preserved in:

`LICENSES/MIT-credo-science.txt`

At the public-repository preparation audit on 2026-09-22 the local file
was byte-identical to the verified upstream Git blob.

## External scientific data

CREDO Analyzer consumes data from external scientific services,
including CREDO and NOAA Space Weather Prediction Center.

These services are data sources. Their use does not mean that their
application source code is incorporated into CREDO Analyzer.

For machine-readable provenance see:

`CODE_PROVENANCE.yaml`
